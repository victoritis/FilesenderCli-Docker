...
from tenacity import RetryCallState, RetryError, retry, stop_after_attempt, wait_fixed, retry_if_exception
import logging
...
def raise_status():
    ...
    try:
        ...
        raise  # Propagate as-is so _handle_error can unwrap the inner exception
...
class FileSenderClient:
    ...
    def __init__(
        ...
        self.http_client = AsyncClient(base_url=self.urls.base, timeout=None, follow_redirects=True)
    ...
    def on_retry(state: RetryCallState) -> None:
        ...
        if isinstance(e, HTTPStatusError):
            try:
                msg = e.response.json().get("message", str(e.response.status_code))
            except Exception:
                msg = str(e.response.status_code)
            logger.warning(f"Intento {state.attempt_number} fallido ({msg}). Reintentando...")
        else:
            logger.warning(f"Intento {state.attempt_number} fallido. Reintentando...")
    ...
    async def download_files(
        ...
        file_meta = list(await self._files_from_token(token))
        if not file_meta:
    ...
    async def download_file(
        ...
        async with self.http_client.stream(
            ...
            if file_name is None:
                ...
                for content_param in content_disposition.split(";"):
